...
namespace LearningBackendAPI.Services
{
    public interface IStudyMaterialService
    {
        ...
        Task<PagedResult<StudyMaterial>> GetAccessibleStudyMaterialsAsync(string userId, string role, string? courseId, string? searchTerm, Dictionary<string, string>? globalFilter, int pageNumber, int pageSize);
