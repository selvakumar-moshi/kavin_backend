...
namespace LearningBackendAPI.Services
{
    public interface IVideoMaterialService
    {
        ...
        Task<PagedResult<VideoMaterial>> GetAccessibleVideoMaterialsAsync(string userId, string role, string? courseId, string? searchTerm, Dictionary<string, string>? globalFilter, int pageNumber, int pageSize);
